...
* `r round(sum(is.na(catch_battle_clear_edi$forkLength))/nrow(catch_battle_clear_edi), 3)*100` % of values in the `forkLength` column are NA.
...
* `r round(sum(is.na(catch_battle_clear_edi$lifeStage))/nrow(catch_battle_clear_edi), 3)*100` % of values in the `lifeStage` column from the EDI data are NA.
...
* `r round(sum(is.na(catch_battle_clear_edi$n))/nrow(catch_battle_clear_edi), 3)*100` % of values in the `n` column in edi data are NA.
...
catch_battle_clear_edi |>
  mutate(source = "EDI") |>
  filter(catchRawID == "{4B85B5E1-9066-496A-A7F52185657FBFDE}") |>
  glimpse()
```
...
<!-- release_fish_battle_clear_db <- release_fish_with_stream |>  -->
<!--   filter(stream == "battle_clear river")  -->
<!-- ``` -->
