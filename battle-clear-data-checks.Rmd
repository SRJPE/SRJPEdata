...
### TODOS:
 - there are data with site=NA and subsite=NA - we do not know which stream it belongs to
 - edi column trap_functioning contains NAs; trap_functioning lookup table does not contain NA
 - map if site == lbc, subsite == lbc; if site ==powerhouse, subsite ==powerhouse (fixed - need to confirm)
...
* `r round(sum(is.na(release_battle_clear_db$lifestage_id))/nrow(release_battle_clear_db), 3)*100` % of values in the `fork_length` column are NA.
