...
The spring run juvenile production estimate (SR JPE) contains a suite of sub-models that rely on different datasets. This document is currently focused on Chinook monitoring data that will be used to train the stock recruit model. 
...
*Gray boxes indicate that monitoring occurred but that data is not being utilized for the stock recruit since a year does not have both adult and juvenile data* 
...
```{r, echo = FALSE}
load(here::here("data", "years_to_exclude_rst_data.rda")) 
...
*Gray boxes indicate that monitoring occurred but that data is not being utilized for the stock recruit. Data may be excluded because there is not adult and juvenile annual overlap or because the data was considered incomplete for a given monitoring type and year.* 
...
The table below compiles all the filtering above to provide a summary of the data utilized by the JPE for each stream and year. 
