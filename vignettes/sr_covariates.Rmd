...
vignette: >
  %\VignetteIndexEntry{SRcovariates}
...
source(here::here("data-raw", "pull_environmental_data.R"))
```

# Selecting Stock Recruit (SR) Environmental Covariates

The goal of this document is to process covariates for use in SR JPE stock recruitment modeling. FlowWest conducted initial literature review and planning to outline covariates that are expected to be important based on past research. This work can be found [here](https://docs.google.com/spreadsheets/d/1Q4VUBE72KdPq0x65y_vUoDMj5dDHp8XwOyDNIyIJKpE/edit#gid=0). FlowWest reviewed covariates with the modeling advisory team to generate the following environmental covariates table to test within the SR model. 
...
This document focus on preparing flow and temperature covariates for initial testing in stock recruit. 
...
std_temp_dd_spawn |> 
  mutate(stream_site = ifelse(is.na(site_group), stream, paste(stream, site_group))) |> 
...
- Note, We currently do not have max daily temperatures for the Sacramento so mainstem is not included
