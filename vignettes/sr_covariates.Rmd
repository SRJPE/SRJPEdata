...
gdd_sac <- standard_temperature |> 
  ...
  group_by(year, gage_agency, gage_number, site_group) |>
  summarise(value = sum(gdd_sac, na.rm = T)) |>
  ungroup() |> 
  mutate(stream = "sacramento river",
         lifestage = "upstream migration",
...
```{r}
gdd_sac |> 
  ggplot(aes(x = year, y = value, color = gage_number)) +
  geom_point() +
  facet_wrap(~stream) +
...
std_temp_weekly <- standard_temperature |> 
  ...
  group_by(week, year, stream, stream_site, gage_agency, gage_number) |> 
  summarize(w_max = max(value, na.rm = T)) |>  # find the max weekly temperature
  ungroup() |>
  group_by(stream_site, stream, year, gage_agency, gage_number) |> 
...
```{r}
std_temp_weekly |> 
  ggplot(aes(x = year, y = weekly_max_temp_max, color = gage_number)) +
  geom_point() +
  facet_wrap(~stream_site) +
  labs(x = "",
...
std_flow_spawn_summary <- standard_flow |> 
  mutate(year = year(date)) |> 
  filter(month(date) %in% c(8:12), stream != "sacramento river") |> 
  group_by(date, year, stream, site_group, gage_agency, gage_number) |> 
...
std_flow_final <- std_flow_spawn_summary |> 
  pivot_wider(id_cols = c(year, stream, site_group, gage_agency, gage_number), values_from = "value", names_from = "statistic") |> 
