...
gdd_sac <- standard_temp |> 
  filter(month(date) %in% migratory_months, 
...
gdd_trib <- standard_temp |>
  filter(month(date) %in% holding_months, 
        stream != "sacramento river",
        statistic == "mean") |>
...
# TODO update format to account for new environmental_gage data object, will have date, site, stream, gage agency, gage number, parameter, statistic, value, unit(?)
...
upstream_passage_timing <- SRJPEdata::upstream_passage  |> 
  filter(!is.na(date),
         ...
         direction %in% c(NA, "up")) |>
  mutate(stream = tolower(stream),
         year = year(date),
         week = week(date)) |>
  group_by(year, stream) |>
  summarise(median_passage_timing = median(week, na.rm = T),
...
upstream_passage_timing |> 
  ...
  facet_wrap(~stream, scales = "free_y") + 
  labs(x = "",
       y = "Median Passage Timing",
       title = "Median Passage Timing By Stream") +
  theme_minimal()
```
...
upstream_passage_index <- SRJPEdata::upstream_passage_estimates |> 
