...
```{r, battle, echo = FALSE}
method_one_hours_fished <- filter(rst_trap, stream %in% 
...
### QC
TODO - deal with NA hours fished value
```{r, echo = FALSE}
...
## Butte ####
method_three_hours_fished <- filter(rst_trap, stream %in% c("butte creek", "yuba river")) %>%
...
## Deer & mill #####
deer_mill_hours_fished <- filter(rst_trap, stream %in% c("deer creek", "mill creek")) %>% 
...
```{r, echo = FALSE}
weekly_effort <- hours_fished_combined %>% 
