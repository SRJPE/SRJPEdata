...
Physical measurements of fish. 

```{r physical-cov-table}
physical_cov_detail <- cov_definitions |> 
...
Candidate covariates were tested within the Stock Recruit and Survival models using leave-one-out (LOO) cross-validation. LOO analysis holds out one brood year at a time and evaluates how well the model predicts that year using the remaining data. This approach was used to:
...
Model performance with covariates is continuously evaluated to ensure covariates keep preforming well. Development of forecast covariates is ongoing. 

Covariates currently selected as of July 2026: 

```{r}
selected_covariates <- tibble::tribble(
  ~Model,          ~Covariate,                  ~Type,         ~Note,
  "Stock-Recruit", "si_gdd_spawn",              "temperature", "Selected for Mill Creek",
  "Stock-Recruit", "si_above_13_temp_week",     "temperature", "Selected for Deer Creek and Yuba River",
  "Stock-Recruit", "si_weekly_max_temp_median", "temperature", "Selected for Clear Creek",
  "Stock-Recruit", "si_mean_flow",              "flow",        "Selected for Battle Creek",
  "Stock-Recruit", "si_min_flow",               "flow",        "Selected for Butte Creek",
  "Survival",      "monthly peak flow",         "flow",        "-",
  "Survival",      "fork_length",               "physical",    "-",
  "PLAD",          "fork_length",               "physical",    "-",
  "In Season",     "null",                      NA,            "-",
  "BTSPASX",       "efficiency_flow",           "flow",        "-",
  "BTSPASX",       "weekly_flow",               "flow",        "-"
)

knitr::kable(selected_covariates)
```
