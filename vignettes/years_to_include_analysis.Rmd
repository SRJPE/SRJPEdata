...
exclude <- tibble(stream = c(rep("battle creek",3), rep("butte creek", 5), rep("deer creek", 9),
                             rep("feather river", 2), rep("mill creek", 5), "sacramento river"),
                  monitoring_year = c(1998, 2007, 2015, 
                                      2019, 2005, 1997, 2006, 1998, 
                                      1992, 1993, 1994, 1997, 1998, 1999, 2006, 2008, 2022,
                                      2021, 2017, 
...
min_max_week <- SRJPEdata::rst_catch |> 
  mutate(monitoring_year = ifelse(week(date) >= 45, year(date) + 1, year(date))) |> 
...
chosen_site_years_to_model <- include |>
  ...
  mutate(exclude = case_when(
                             monitoring_year == 1998 & site == "ubc" ~ T, 
                             monitoring_year == 2003 & site == "ucc" ~ T,
                             monitoring_year == 2003 & site == "yuba river" ~ T,
                             ...
                             monitoring_year == 2002 & site == "knights landing" ~ T,
                             T ~ F),
         site_group = case_when(site %in% lfc_sites ~ "upper feather lfc",
                                site %in% hfc_sites ~ "upper feather hfc",
                                T ~ NA)) |> 
  filter(exclude == F) |> 
  select(monitoring_year, stream, site, min_date, min_week, max_date, max_week) |> 
...
#TODO upstream passage is still empty! Need to fix 
standard_upstream <- SRJPEdata::upstream_passage
...
upstream_weekly <- upstream_summary %>% glimpse()
  # add catch_date_week to make sure all weeks are included in the data
