...
heat_map <- ggplot(plot_data, aes(x = fake_date, y = wy, fill = n_streams, text = paste0(date_range, "<br>", streams))) +
  ...
  ylab('water year') +
  xlab("") + 
  scale_fill_viridis_b(n.breaks = 8, direction = -1) +
  labs(fill = "Number of streams")
plotly::ggplotly(heat_map) 
# heat_map
```

### Years to exclude - Historical Ruleset
...
### Years to exclude - Current Ruleset

Ruleset defined as "at least 19 weeks included within a run year (week 45-53 and 1-22) for a given site" or 60% of weeks sampled. Sites that do not have any (across all years) mark-repcature efficiency trials are also excluded. Additionally, years with less than 3 weeks with catch > 0 are excluded.

```{r, echo = FALSE}
years_exclude_nice_names <- SRJPEdata::rst_model_years |>
  filter(exclude == T) |> 
  select(
    "Stream" = stream,
    "Site" = site,
...
*... with `r nrow(years_exclude_nice_names) - 10` more rows*
...
```{r, echo = FALSE}
knitr::kable(SRJPEdata::rst_model_years |>
    filter(exclude == T) |>  
    filter(grepl("spring run specific", exclusion_type)))
```

### rst_model_years object

RST modeling years data object can be found at `SRJPEdata::rst_model_years`
...
We utilized strategies to come up with a list of years to exclude from modeling for adult data. See a section of the `adult_model_years`table filtered to show years to exclude below.

```{r, echo = FALSE}

years_exclude_nice_names_adult <- SRJPEdata::adult_model_years |> 
  filter(exclude == T) |> select("Stream" = stream, 
                                "Year" = year, 
                                "Data Type" = data_type,
...
## adult_model_years object

Adult modeling years data object can be found at `SRJPEdata::adult_model_years`
