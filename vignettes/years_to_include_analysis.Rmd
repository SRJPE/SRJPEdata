...
```{r}
passage_include <- SRJPEdata::upstream_passage_estimates |> 
  distinct(stream, year) |> 
  full_join(years_to_exclude_adult|> 
              filter(data_type == "upstream passage") |> 
              select(year, stream) |> 
              mutate(exclude = T)) |> 
  filter(is.na(exclude)) |> 
  select(-exclude) |> 
...
carcass_include <- SRJPEdata::carcass_estimates |> 
  distinct(stream, year) |> 
  full_join(years_to_exclude_adult|> 
              filter(data_type == "carcass") |> 
              select(year, stream) |> 
              mutate(exclude = T)) |> 
  filter(is.na(exclude)) |> 
  select(-exclude) |> 
...
holding_include <- SRJPEdata::holding |> 
  mutate(year = year(date)) |> 
  distinct(stream, year) |> 
  full_join(years_to_exclude_adult|> 
              filter(data_type == "holding") |> 
              select(year, stream) |> 
              mutate(exclude = T)) |> 
  filter(is.na(exclude)) |> 
  select(-exclude) |> 
...
years_to_include_adult <- bind_rows(passage_include,
                                    carcass_include,
                                    redd_include,
                                    holding_include)
knitr::kable(head(include, 10))
