...
```{r, echo = FALSE}
years_to_exclude_rst_data <- years_exclude_rst_csv |> 
  distinct()


# Automatic exclusion criteria to be applied to most recent year
max_old_year <- max(years_to_exclude_rst_data$run_year)
# Consider thinking about adding subsites together, right now it looks like subsites are fished equally
new_years_to_exclude <- rst_trap |>
  mutate(
    week = ifelse(
      is.na(trap_stop_date),
      week(trap_start_date),
      week(trap_stop_date)
    ),
    year = ifelse(
      is.na(trap_stop_date),
      year(trap_start_date),
      year(trap_stop_date)
    ),
  ...
  ungroup() |>
  mutate(
    exclude = ifelse(number_weeks < 20, TRUE, FALSE),
    exclusion_type = "automatic",
    reason_for_exclusion = "less than 75% of weeks sampled"
  ) |>
  filter(exclude) |>
  select(stream, site, run_year = monitoring_year, exclusion_type, reason_for_exclusion) 
...
current_run_year <- if (month(current_date) %in% 10:12) {
  year(current_date) + 1
} else {
  year(current_date)
}

years_to_include_rst_data <- rst_catch |> 
  ...
  full_join(years_to_exclude_rst_data |> 
              ...
              mutate(exclude = T)) |> 
  mutate(current_year = ifelse(month(current_date) %in% c(10:12, 1:5) & run_year == current_run_year, T, F)) |> 
  filter(is.na(exclude) & current_year == F) |> 
  select(-c(exclude, current_year))
