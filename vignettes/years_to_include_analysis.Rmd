...
-   Exclude year if survey does not cover core reaches in a year/if less than 50% of reaches are sampled. 
...
-   Exclude year if flows exceed X value on each tributary (overpass weir etc..) (TODO)
...
The cumulative catch curve below shows cumulative upstream passage throughout time for Battle Creek. We used simmilar plots on each tributaries to exclude years where there is very limited sampling. This plot shows that in 2019 and 2006, upstream passage monitoring started late in the season on Battle Creek. See additional [cumulative catch curves in shiny](https://flowwest.shinyapps.io/jpe-rst-workshop-shiny/) for each tributary.
...
The below heat map shows all streams. \_what does it show.\_ Some video passage systems have continued footage throughout the season. Some years there are gaps within season for specific systems.
...
standard_redd <- read_csv(here::here("data-raw", "database-tables", "standard_daily_redd.csv"))
...
years_exclude_adult <- read_csv(here::here("data-raw", "helper-tables", "years_to_exclude_adult_datasets.csv"))
years_exclude_nice_names_adult <- years_exclude_adult |> select("Stream" = stream, 
                                                    "Year" = year, 
                                                    "Data Type" = data_type,
