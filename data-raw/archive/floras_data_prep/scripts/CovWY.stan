...
parameters {
  ...
  vector <lower=-10,upper=10> [NS_bCov] S_bCov; 
...
generated quantities{
  ...
  SurvForecast[1]=  inv_logit(S_bReach[1] + s_re)^RmultSac * 
  inv_logit(S_bReach[2] + s_re)^Rmult[1,2] *
  inv_logit(S_bReach[3] + s_re)^Rmult[1,3];
  for(j in 1:NS_bCov){
    SurvForecast[j+1]= inv_logit(S_bReach[1] + S_bCov[j] + s_re)^RmultSac * 
    inv_logit(S_bReach[2] + S_bCov[j] + s_re)^Rmult[1,2] *
  ...
  if(UseSizeEffect == 1){
    for(ix in 1:Nsz){
      SurvForecastSz[ix,1]= inv_logit(S_bReach[1] + S_bSz*Xsz[ix] + s_re)^RmultSac * 
      inv_logit(S_bReach[2] + S_bSz*Xsz[ix] + s_re)^Rmult[1,2] *
      inv_logit(S_bReach[3] +  S_bSz*Xsz[ix] + s_re)^Rmult[1,3];
      for(j in 1:NS_bCov){
        SurvForecastSz[ix,j+1]= inv_logit(S_bReach[1] +  S_bCov[j] + S_bSz*Xsz[ix] + s_re)^RmultSac * 
        inv_logit(S_bReach[2] + S_bCov[j] + S_bSz*Xsz[ix] + s_re)^Rmult[1,2] *
  ...
  if(UseSizeEffect == 1){
    for(ix in 1:Nsz){
      for(itrib in 1:Ntribs){
        TribSurvForecastSz[ix,1,itrib]=(inv_logit(S_bTrib[itrib] + s_ret)^RmultTrib[itrib]); 
        for(j in 1:NS_bCov){
