...
for(irun in 1:9){
  ModNm <- switch(irun,"NoCov","CovWY","CovWY","CovWY","CovWY","CovWY","CovWY","CovWY","CovWY") #"CovInd","CovInd","CovInd"
  fnext <- ""
  ...
  if (ModNm == "NoCov"){
    ...
    ParSaveList <- c("P_b","muPb","sdPb","S_bReach","S_bTrib","S_bCov","S_bCovT","RE_sd","RE_sdT","S_RE","S_REt",
...
# }
  ...
  fit <- stan(file=paste0("scripts/",ModNm,".stan"),data=model_data, init=inits, chains=nchains,
              iter=niter,include=T,pars=ParSaveList)#,algorithm = "Fixed_param",sample_file="post.txt",diagnostic_file="diagnostic.txt" #algorithm = "Fixed_param"
