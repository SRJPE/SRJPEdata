...
monthly_mean <- standard_temperature |>
  filter(
    gage_number != "LCC" &
      !site_group %in% c("upper feather hfc", "lower feather river")
  ) |> # UCC more representative of spawning; LFC more representative of spawning
  mutate(
...
std_temp_dd_spawn <- standard_temperature |>
  filter(
    gage_number != "LCC" &
      !site_group %in% c("upper feather hfc", "lower feather river")
  ) |> # UCC more representative of spawning
  mutate(
    year = year(date),
    rm = case_when(
      stream %in% c("feather river") & year == 1997 ~ "incomplete",
  ...
  arrange() |>
  mutate(
    rolling_3_day_mean = rollapply(value, 3, mean, align = "center", fill = NA),
  ...
  filter(
    month(date) %in% c(8:12),
    stream != "sacramento river",
    rm != "incomplete"
  ) |> # spawning/incubation; tributaries only
  select(date, year, stream, statistic, value) |>
  pivot_wider(
    id_cols = c(date, year, stream),
    values_from = "value",
  ...
  summarize(gdd_spawn = sum(max_mean, na.rm = T)) |>
  glimpse()
...
std_temp_7dadm <- standard_temperature |>
  filter(
    gage_number != "LCC" &
      !site_group %in% c("upper feather hfc", "lower feather river", "tisdale")
  ) |>
...
std_temp_7dadm_final <- std_temp_7dadm_above_13 |>
  ...
  select(stream, above_13_temp_day, above_13_temp_week, year) |>
  glimpse()
...
std_temp_weekly <- standard_temperature |>
  ...
  summarize(
    weekly_max_temp_max = max(w_max, na.rm = T),
    weekly_max_temp_mean = mean(w_max, na.rm = T),
    weekly_max_temp_median = median(w_max, na.rm = T)
  ) |>
  glimpse()
...
std_flow_rear_combined <- bind_rows(
  std_flow_rear_summary,
  std_flow_rear_sac_summary
)
...
stock_recruit_covariates <- std_temp_dd_spawn |>
  ...
  pivot_longer(
    cols = c(
      ...
      above_13_temp_week,
      weekly_max_temp_max,
      weekly_max_temp_mean,
      weekly_max_temp_median,
      mean_flow,
      max_flow,
      min_flow,
      median_flow
...
si_min_flow <- sr_covariates |>
  filter(
    lifestage == "spawning and incubation",
    covariate_structure == "min_flow"
  ) |>
  as_tibble() |>
  mutate(name = "si_min_flow", water_year = NA) |>
  select(name, year, water_year, stream, value)

si_max_flow <- sr_covariates |>
  filter(
    lifestage == "spawning and incubation",
    covariate_structure == "max_flow"
  ) |>
  as_tibble() |>
  mutate(name = "si_max_flow", water_year = NA) |>
  select(name, year, water_year, stream, value)
...
si_max_temp <- sr_covariates |>
  filter(
    lifestage == "spawning and incubation",
    covariate_structure == "weekly_max_temp_max"
  ) |>
  as_tibble() |>
  mutate(name = "si_max_temp", water_year = NA) |>
  select(name, year, water_year, stream, value)

above_13 <- sr_covariates |>
  filter(covariate_structure == "above_13_temp_week") |>
  as_tibble() |>
  mutate(name = "above_13", water_year = NA) |>
  select(name, year, water_year, stream, value)
...
# join on stream.
rs_keswick_monthly_raw <- cdec_query(
  station = "KES",
  sensor = "15",
  dur_code = "M",
...
rs_shasta_monthly_raw <- dataRetrieval::readNWISdv(
  siteNumbers = 11370000,
  statCd = 32400,
  parameterCd = "00054",
...
peak_flow_monthly <- flow_daily |>
  ...
  mutate(month = month(date), year = year(date), name = "monthly_max_flow") |>
...
wy <- wy_raw |>
  filter(!is.na(WY)) |>
  mutate(
    wy_type = case_when(WYT %in% c("D", "BN", "AN") ~ "D/BN/AN", TRUE ~ WYT),
    year = NA,
    water_year = WY,
...
feather_post_2023 <- CDECRetrieve::cdec_query(
  station = "TFB",
  dur_code = "H",
  sensor_num = "20",
  ...
  mutate(
    parameter_value = ifelse(parameter_value < 0, NA_real_, parameter_value)
  ) |>
...
water_year_types <- mean_discharge_by_site |>
  ...
  mutate(
    ...
    water_year_type = case_when(
      ...
      exceedance_probability > 33.3 &
        exceedance_probability <= 66.3 ~ "Average",
      TRUE ~ "Dry"
    )
  )
...
flow_water_year_type <- water_year_types |>
  mutate(
    name = "3_category_flow_exceedance_year_type",
    year = NA,
    stream = site_name,
