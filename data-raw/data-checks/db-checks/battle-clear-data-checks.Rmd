...
**- Findings catch:**
  ...
  - a lot of "not provided" values on EDI lifestage. Since DB has not been wrangled it is hard to compare. Flagging this in case we cant to look into details
...
- Date ranges are different- it looks like a few dates in database jump from 2025-07-08 to 2025-12-20
...
*EDI `trap_start_date`*
```{r}
range(trap_battle_clear_edi$trap_start_date, na.rm = TRUE)
```
...
* `r round(sum(is.na(release_battle_clear_edi$origin_released))/nrow(release_battle_clear_db), 3)*100` % of values in the `origin_released` column are NA.
