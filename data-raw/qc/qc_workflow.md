...
Alert level, severity, and a one-line description of what each check measures are looked up from `CHECK_METADATA` in `qc_helpers.R` (a single `(data_type, issue_type, field) -> (alert_level, severity, description)` table), rather than duplicating each category report's hardcoded assignment a third time.

Every check is computed at the `(stream, run_year)` grain first (`*_by_stream` tibbles in the `rst-metrics`/`flow-metrics`/`efficiency-metrics` chunks), then pooled up to the `run_year`-only grain used everywhere except the per-stream heatmap tabs. Pooling sums `n_evaluated`/`n_failed` across streams (`pool_across_streams()` in the setup chunk) rather than recomputing from source data a second time, so the pooled and per-stream views can't drift apart from each other.

Contents, in report order:
- **Failure rate heatmap** (interactive, plotly; first — the report's at-a-glance entry point; everything after it gives exact numbers for whatever this surfaces) — checks (rows) x run_years (columns) from `MIN_TREND_YEAR` (1990) onward, colored by `% failed` and split into an **Error** panel (red hues) and a **Warning** panel (yellow hues) by `alert_level`, since the two mean different things and shouldn't sit on one shared color scale (each panel is `ggplotly()`'d separately and combined with `plotly::subplot()`, since a single `facet_wrap()` can't carry two independent color scales, and a static combiner like `patchwork` can't carry a hover tooltip). `review_run_year` column outlined. Hovering a tile shows the exact numbers and the check's description, same as the trend plot below. Presented as tabs: **Overall** (pooled across streams) plus one tab per stream (recomputed from `metrics_by_stream`), so a problem visible in the pooled view can be traced to whether it's basin-wide or concentrated at one site.
...
- **Trend plot** (interactive, plotly) — one line per check per `data_type`, `% failed` across every run_year from `MIN_TREND_YEAR` onward, with `review_run_year` ringed on each line. Hovering a point shows the exact numbers, alert level/severity, and the check's description — this is the main view for telling whether `review_run_year` continues a multi-year trend or is an isolated blip, which the headline table and heatmap can't show as directly since one collapses history into an average and the other needs a scan rather than a continuous line.
...
- **All open issues** — `data_type x alert_level x severity` counts of every currently-`open` issue in `qc_log.csv`, across all run_years
- **Error log for `review_run_year`** — the individual error-level (`alert_level = "error"`) rows from `qc_log.csv` for the year being reviewed, rendered as a static `kableExtra` table with `open`-status rows highlighted, so the reviewer-facing detail behind the computed rates above is one scroll away rather than a separate file
