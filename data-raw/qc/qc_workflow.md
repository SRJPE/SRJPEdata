...
- **Trend plot** (interactive, plotly) — one line per check per `data_type`, `% failed` across every run_year from `MIN_TREND_YEAR` (1990) onward, with `review_run_year` ringed on each line. Hovering a point shows the exact numbers, alert level/severity, and the check's description — this is the main view for telling whether `review_run_year` continues a multi-year trend or is an isolated blip, which the headline table and dumbbell plot can't show since both collapse history into one average.
- **Failure rate heatmap** — checks (rows) x run_years (columns) from `MIN_TREND_YEAR` onward, colored by `% failed`, `review_run_year` column outlined. Presented as tabs: **Overall** (pooled across streams) plus one tab per stream (recomputed from `metrics_by_stream`), so a problem visible in the pooled view can be traced to whether it's basin-wide or concentrated at one site.
