...
2. Fitting a linear regression models for mean, min, and max temperatures 

3. Make predictions for new dataset using the fitted models

4. Combine predictions with actual measurements and handle missing values
...
* Pulling in gage data from (GRL will represent the High Flow Channel (HFC) and FRA will represent the Low Flow Channel (LFC):
...
* Prepare datasets for regression analysis - dataset with no missing data (Butte Creek data) to train and dataset with missing data to predict (Feather River)
 ...
 * Pulling in gage data from YR7 CDEC gage
 * YR7 data available from 2020 onwards 
 * Note: Temperature data collected along with RST data was originally used to fill the gap prior to 2020; however, due to inconsistencies in these data sources the resulting predicted mean values were lower than the min values as the RST data only has mean data. 
 * Prepare datasets for regression analysis - dataset with no missing data (Butte Creek data) to train and dataset with missing data to predict (Yuba River)
