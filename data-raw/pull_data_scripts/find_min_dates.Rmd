---
title: "Finding salmon minimum data for environmetal data pull"
...
Summary of minimum date ranges:
...
*rst_catch*
```{r}
butte_catch <- rst_catch |> 
  filter(stream == "butte creek") |> 
  glimpse()

range(butte_catch$date, na.rm = TRUE) # 1995 2022
```

*carcass_estimate*
```{r}
butte_carcass <- carcass_estimates |> 
  filter(stream == "butte creek") |> 
  glimpse()

range(butte_carcass$year) #2012 2022
```

*holding*
```{r}
butte_holding <- holding |> 
  filter(stream == "butte creek") |> 
  glimpse()

range(butte_holding$date) # "2001-08-14" "2017-07-13"
```

No *redd* on Butte Creek

No *upstream_passage_estimates* on Butte
