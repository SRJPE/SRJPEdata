...
cwt_distances <- read_csv(
  "data-raw/helper-tables/cwt_data/cwt_distances_summary.csv"
) |>
  add_row(
    Id = 8,
  ...
  mutate(
    delta_distance = round(distance_rkm, 2),
...
site_info_feather <- read_csv(
  "data-raw/helper-tables/cwt_data/CWT_release_sites_latlong.csv"
) |>
  filter(hatchery_location_name == "FEATHER R HATCHERY") |>
  left_join(cwt_distances, by = "release_location_name") |>
  rename(release_latitude = latitude, release_longitude = longitude) |>
  glimpse()

# read in lat/longs from https://www.rmpc.org/data-selection/rmis-queries/
rest_of_site_info <- read_csv(here::here(
  "data-raw",
  "helper-tables",
  "cwt_data",
  "CSV55526.TXT"
...
cwt_data_raw_historical <- read_csv(
  "data-raw/helper-tables/cwt_data/CWT_releases.csv"
) |>
  mutate(
...
cwt_data_raw_2023 <- read_csv(
  "data-raw/helper-tables/cwt_data/CWT_releases_2023_2025.csv"
) |>
  mutate(
  ...
  mutate(
    date_span = 1 +
      as.numeric(difftime(
        ...
        first_release_date,
        units = "days"
      )), # can be used to summarize conditions over release period
    total_marked_N = round(
      (cwt_1st_mark_count + cwt_2nd_mark_count) * (1 - tag_loss_rate)
    ),
    total_unmarked_N = non_cwt_1st_mark_count +
      non_cwt_2nd_mark_count +
      round((cwt_1st_mark_count + cwt_2nd_mark_count) * (tag_loss_rate)),
    total_release_N = cwt_1st_mark_count +
      cwt_2nd_mark_count +
      non_cwt_1st_mark_count +
      non_cwt_2nd_mark_count,
    mark_rate = total_marked_N / total_release_N, # will serve as expansion factor for catch analysis
    mid_release_date = first_release_date + (date_span / 2)
  ) |> # use to apply conditions for group
  left_join(
    site_info |> select(-hatchery_location_name),
    by = "release_location_name"
  ) |> # left_join delta_distance, release_latitude, release_longitude columns
...
max_flows <- SRJPEdata::forecast_covariates |>
  filter(
    ...
    !(stream == "sacramento river" & site_group != "red bluff diversion dam")
  ) |>
...
release_location_stream_lookup <- read_csv(here::here(
  "data-raw",
  "helper-tables",
  "cwt_data",
  "release_location_lookup.csv"
)) |>
...
hatchery_release_all <- cwt_data_summary_all |>
  ...
  mutate(
    ...
    water_year = ifelse(
      month %in% 10:12,
      year(release_date_use) + 1,
      year(release_date_use)
    )
  ) |>
  ungroup() |>
...
feather_hatchery_release <- hatchery_release_all |>
  filter(
    species == 1,
    run == 1,
    release_location_name %in%
      c(
        "FEATHER THERMALITO BYPASS",
        "YUBA AT HALLWOOD BLVD",
        "YUBA RIVER",
        "FEATHER BOYDS PUMP RAMP",
        "FEATHER AT LIVE OAK",
        "FEATHER AT GRIDLEY",
        "FEATHER AT YUBA CITY",
        "FEATHER BEL THRM HI FLOW",
        "FEATHER R HATCHERY"
      ),
...
# RST recaptures ----------------------------------------------------------
recaptures_raw <- readxl::read_excel(here::here(
  "data-raw",
  "helper-tables",
  "cwt_data",
  "KL.CWT.Data.1999-2024.UPDATED.1.xlsx"
)) |>
  janitor::clean_names()

rst_cwt_recaptures <- recaptures_raw |>
  mutate(
    date_chr = str_trim(date),
    date_mdy = suppressWarnings(mdy(date_chr, quiet = TRUE)),
    date_excel = suppressWarnings(as_date(
      as.numeric(date_chr),
      origin = "1899-12-30"
    )),
    date_parsed = case_when(
      str_detect(date_chr, "^[0-9]+(\\.[0-9]+)?$") &
        is.na(date_mdy) ~ date_excel,
      TRUE ~ date_mdy
    )
  ) |>
  select(date = date_parsed, forklength = fl, weight = wt, tag_code = cwt) |>
  mutate(stream = "sacramento river", site = "knights landing") |>
...
knight_distances <- read_csv(
  "data-raw/helper-tables/knights_landing_cwt_distances.csv"
)
...
hatchery_release <- knight_distances |>
  ...
  mutate(
    release_to_knights_landing_distance = dist_along_mi * 1.60934, # change to river km
    species = "chinook salmon",
    run = case_when(
      run == 1 ~ "spring",
      run == 3 ~ "fall",
    ...
    stream = case_when(
      hatchery_location_name == "COLEMAN NFH" ~ "sacramento river",
      T ~ stream
    )
  ) |>
  left_join(
