...
cwt_distances <- read.csv("data-raw/cwt_data/cwt_distances_summary.csv") |>
  add_row(Id = 8,
...
site_info <- read.csv("data-raw/cwt_data/CWT_release_sites_latlong.csv") |> 
...
# This dataset is fro are from the Regionglimpse()# This dataset is fro are from the Regional Mark Processing Center: https://www.rmpc.org/
cwt_data_raw <- read.csv("data-raw/cwt_data/CWT.Sac.Valley.Releases.csv") 
...
cwt_data_summary <- cwt_data_raw |> 
  ...
  mutate(
    ...
    wy_first_release = as.Date(first_release_date, format = "%m/%d/%Y"), # can use to filter for problem groups later on
    wy_last_release = as.Date(last_release_date, format = "%m/%d/%Y"),   # can use to filter for problem groups later on
...
write.csv(cwt_data_summary, "data-raw/cwt_data/cwt_data_summary.csv")

cwt_data_grouped <- cwt_data_summary |> 
  group_by(
    release_location_name,
    avg_weight,
    avg_length,
...
write.csv(cwt_data_grouped, "data-raw/cwt_data/cwt_data_grouped.csv")
