...
# Pull in Trap table 
try(rst_trap_query_pilot <-  dbGetQuery(con, "SELECT 
                                    t.id as trap_visit_id,
                                    p.stream_name as stream,
                                    tl.site_name as site,
                                    tl.trap_name as trap_name,
                                    t.is_paper_entry, 
                                    t.trap_visit_time_start as trap_visit_time_restart, 
                                    t.trap_visit_time_end,
                                    fp.definition as fish_processed,
                                    wfnp.definition as why_fish_not_processed,
                                    e.definition as sample_gear,
                                    t.cone_depth,
                                    tf.definition as trap_functioning,
                                    wtnf.definition as why_trap_not_functioning,
                                    tsae.definition as trap_status_at_end, 
                                    t.total_revolutions, 
                                    t.rpm_at_start, 
                                    t.rpm_at_end, 
                                    t.in_half_cone_configuration,
                                    t.debris_volume_gal 
                                   FROM trap_visit t
