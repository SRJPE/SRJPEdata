...
**- Findings trap:**
  ...
  - EDI has earlier date range, starting 2003. DB starts 2021
...
```{r echo=FALSE}
range(trap_knights_db$trap_visit_time_start)
```

`trap_visit_time_end`
```{r echo=FALSE}
range(trap_knights_db$trap_visit_time_end)
```
...
* `r round(sum(is.na(trap_knights_db$waterTemp))/nrow(trap_knights_db), 3)*100` % of values in the `waterTemp` column in the database dataset are NA.
```{r echo=FALSE}
summary(trap_knights_db$waterTemp)
```

EDI

* `r round(sum(is.na(trap_knights_edi$waterTemp))/nrow(trap_knights_edi), 3)*100` % of values in the `waterTemp` column in the EDI dataset are NA.
```{r echo=FALSE}
summary(trap_knights_edi$waterTemp)
```

*turbidity*

DB
* `r round(sum(is.na(trap_knights_db$turbidity))/nrow(trap_knights_db), 3)*100` % of values in the `turbidity` column in the database dataset are NA.
```{r echo=FALSE}
summary(trap_knights_db$turbidity)
```

EDI
* `r round(sum(is.na(trap_knights_edi$turbidity))/nrow(trap_knights_edi), 3)*100` % of values in the `turbidity` column in the database dataset are NA.
```{r echo=FALSE}
summary(trap_knights_edi$turbidity)
```
...
range(release_knights_db$date_released)
```

column names
```{r echo=FALSE}
colnames(release_knights_db)
```
...
range(release_knights_edi$releaseTime, na.rm = TRUE)
```

column names
```{r echo=FALSE}
colnames(release_knights_edi)
```

*number_released or nReleased*

DB
* `r round(sum(is.na(release_knights_db$number_released))/nrow(release_knights_db), 3)*100` % of values in the `number_released` column in the database dataset are NA.
```{r echo=FALSE}
summary(release_knights_db$number_released)
```
EDI
* `r round(sum(is.na(release_knights_edi$nReleased))/nrow(release_knights_edi), 3)*100` % of values in the `nReleased` column in EDI dataset are NA.
```{r echo=FALSE}
...
ggplot() +
  geom_point(data = release_knights_db_clean,
              aes(x = date, y = number_released),
              color = "blue", alpha = 0.4, shape = 16, size = 2) +
  geom_point(data = release_knights_edi_clean,
              aes(x = date, y = nReleased),
              color = "red", alpha = 0.4, shape = 17, size = 2) +
  theme_minimal() +
  labs(x = "Date",
...
**- Findings:**
   - EDI temporal coverage starts at earlier year than DB. Earliest year is 2007
   - there are only 9 data entries on DB
   - DB lifestages are mostly just "5" and some "6"
...
```{r include=FALSE}
recapture_knights_edi <- read_csv("data-raw/edi-zips/knights_landing_recapture.csv") |> glimpse()

range(recapture_knights_edi$visitTime, na.rm = TRUE)
```

column names DB
```{r echo=FALSE}
colnames(recapture_knights_db)
```

column names EDI
```{r echo=FALSE}
colnames(recapture_knights_edi)
```

*date vs count/number_recapture*

* `r round(sum(is.na(recapture_knights_db$count))/nrow(recapture_knights_db), 3)*100` % of values in the `count` column from the database are NA.

* `r round(sum(is.na(recapture_knights_edi$n))/nrow(recapture_knights_edi), 3)*100` % of values in the `n` column from EDI are NA.

```{r echo=FALSE, warning=FALSE}
recapture_knights_db <- recapture_knights_db |>
  mutate(source = "DB")

recapture_knights_edi <- recapture_knights_edi |>
  mutate(source = "EDI",
         date = as.Date(visitTime),
         count = n) 

recapture_knights_combined <- bind_rows(recapture_knights_edi, recapture_knights_db)

ggplot(recapture_knights_combined, aes(x = date, y = count, color = source, shape = source)) +
  geom_point(alpha = 0.4, width = 10, height = 0.5, size = 2) +
  theme_minimal() +
  labs(x = "Date Recaptured",
       y = "Count - Number Recaptured",
       color = "Source",
       shape = "Source",
       title = "Recaptures by Source (Sacramento River - Knights Landing)")
```

*fork_length*

* `r round(sum(is.na(recapture_knights_db$fork_length))/nrow(recapture_knights_db), 3)*100` % of values in the `fork_length` column from the database are NA.

* `r round(sum(is.na(recapture_knights_edi$forkLength))/nrow(recapture_knights_edi), 3)*100` % of values in the `forkLength` column from EDI are NA.

*lifestage*

* `r round(sum(is.na(recapture_knights_db$lifestage_id))/nrow(recapture_knights_db), 3)*100` % of values in the `lifestage_id` column from the database are NA.

* `r round(sum(is.na(recapture_knights_edi$lifeStage))/nrow(recapture_knights_edi), 3)*100` % of values in the `lifeStage` column from EDI are NA.
...
recapture_knights_db |> 
  mutate(source = "DB") |>
  filter(catch_raw_id == "160190") |>
  glimpse()

recapture_knights_edi |> 
  mutate(source = "EDI") |>
  filter(catchRawID == "160190") |>
  glimpse()
```
...
recapture_knights_db |> 
  mutate(source = "DB") |>
  filter(catch_raw_id == "160506") |>
  glimpse()

recapture_knights_edi |> 
  mutate(source = "EDI") |>
  filter(catchRawID == "160506") |>
  glimpse()
```
...
recapture_knights_db |> 
  mutate(source = "DB") |>
  filter(catch_raw_id == "152127") |>
  glimpse()

recapture_knights_edi |> 
  mutate(source = "EDI") |>
  filter(catchRawID == "152127") |>
  glimpse()
```
...
recapture_knights_db |> 
  mutate(source = "DB") |>
  filter(catch_raw_id == "152129") |>
  glimpse()

recapture_knights_edi |> 
  mutate(source = "EDI") |>
  filter(catchRawID == "152129") |>
  glimpse()
```
...
catch_tisdale_db <- catch_with_stream |> 
  filter(trap_location_id %in% c(55,56,57))  # trap id for Sacramento/Tisdale landing
...
**- Findings trap:**
  - EDI has earlier date range, starting 2012. DB starts 2022
...
`trap_visit_time_start`
```{r echo=FALSE}
range(trap_tisdale_db$trap_visit_time_start)
```

`trap_visit_time_end`
```{r echo=FALSE}
range(trap_tisdale_db$trap_visit_time_end)
```
...
* `r round(sum(is.na(trap_tisdale_db$water_temp))/nrow(trap_tisdale_db), 3)*100` % of values in the `water_temp` column in the database dataset are NA.
```{r echo=FALSE}
summary(trap_tisdale_db$water_temp)
```

EDI

* `r round(sum(is.na(trap_tisdale_edi$waterTemp))/nrow(trap_tisdale_edi), 3)*100` % of values in the `waterTemp` column in the EDI dataset are NA.
```{r echo=FALSE}
summary(trap_tisdale_edi$waterTemp)
```

*turbidity*

DB
* `r round(sum(is.na(trap_tisdale_db$turbidity))/nrow(trap_tisdale_db), 3)*100` % of values in the `turbidity` column in the database dataset are NA.
```{r echo=FALSE}
summary(trap_tisdale_db$turbidity)
```

EDI
* `r round(sum(is.na(trap_tisdale_edi$turbidity))/nrow(trap_tisdale_edi), 3)*100` % of values in the `turbidity` column in EDI dataset are NA.
```{r echo=FALSE}
summary(trap_tisdale_edi$turbidity)
```
...
```{r echo=FALSE}
summary(release_tisdale_db$number_released)
```
EDI
* `r round(sum(is.na(release_tisdale_edi$nReleased))/nrow(release_tisdale_edi), 3)*100` % of values in the `nReleased` column in EDI dataset are NA.
```{r echo=FALSE}
...
ggplot() +
  geom_point(data = release_tisdale_db_clean,
              aes(x = date, y = number_released),
              color = "blue", alpha = 0.4, shape = 16, size = 2) +
  geom_point(data = release_tisdale_edi_clean,
              aes(x = date, y = nReleased),
              color = "red", alpha = 0.4, shape = 17, size = 2) +
  theme_minimal() +
  labs(x = "Date",
...
```{r echo=FALSE}
release_tisdale_db |>  
  filter(release_id == "294") |> 
  glimpse()

release_tisdale_edi |> 
  filter(releaseID == "294") |> 
  glimpse()
```
...
```{r echo=FALSE}
release_tisdale_db |>  
  filter(release_id == "296") |> 
  glimpse()

release_tisdale_edi |> 
  filter(releaseID == "296") |> 
  glimpse()
```  
...
```{r echo=FALSE}
release_tisdale_db |>  
  filter(release_id == "318") |> 
  glimpse()

release_tisdale_edi |> 
  filter(releaseID == "318") |> 
  glimpse()
```  
...
**- Findings:**
   - All fork length values are NA in database
   - there are only 8 data entries in database
...
```{r include=FALSE}
recapture_tisdale_edi <- read_csv("data-raw/edi-zips/tisdale_recapture.csv") |> glimpse()

range(recapture_tisdale_edi$visitTime, na.rm = TRUE)
```

column names DB
```{r echo=FALSE}
colnames(recapture_tisdale_db)
```

column names EDI
```{r echo=FALSE}
colnames(recapture_tisdale_edi)
```

  
*date vs count/number_recapture*

* `r round(sum(is.na(recapture_tisdale_db$count))/nrow(recapture_tisdale_db), 3)*100` % of values in the `count` column from the database are NA.

* `r round(sum(is.na(recapture_tisdale_edi$n))/nrow(recapture_tisdale_edi), 3)*100` % of values in the `n` column from EDI are NA.

```{r echo=FALSE, warning=FALSE}
recapture_tisdale_db <- recapture_tisdale_db |>
  mutate(source = "DB")

recapture_tisdale_edi <- recapture_tisdale_edi |>
  mutate(source = "EDI",
         date = as.Date(visitTime),
         count = n) 

recapture_tisdale_combined <- bind_rows(recapture_tisdale_edi, recapture_tisdale_db)

ggplot(recapture_tisdale_combined, aes(x = date, y = count, color = source, shape = source)) +
  geom_point(alpha = 0.4, width = 10, height = 0.5, size = 2) +
  theme_minimal() +
  labs(x = "Date Recaptured",
       y = "Count - Number Recaptured",
       color = "Source",
       shape = "Source",
       title = "Recaptures by Source (Sacramento River - Tisdale)")
```

*fork_length*

* `r round(sum(is.na(recapture_tisdale_db$fork_length))/nrow(recapture_tisdale_db), 3)*100` % of values in the `fork_length` column from the database are NA.

* `r round(sum(is.na(recapture_tisdale_edi$forkLength))/nrow(recapture_tisdale_edi), 3)*100` % of values in the `forkLength` column from EDI are NA.

*lifestage*

* `r round(sum(is.na(recapture_tisdale_db$lifestage_id))/nrow(recapture_tisdale_db), 3)*100` % of values in the `lifestage_id` column from the database are NA.

* `r round(sum(is.na(recapture_tisdale_edi$lifeStage))/nrow(recapture_tisdale_edi), 3)*100` % of values in the `lifeStage` column from EDI are NA.
...
recapture_tisdale_db |> 
  mutate(source = "DB") |>
  filter(catch_raw_id == "88914") |>
  glimpse()

recapture_tisdale_edi |> 
  mutate(source = "EDI") |>
  filter(catchRawID == "88914") |>
  glimpse()
```
...
recapture_tisdale_db |> 
  mutate(source = "DB") |>
  filter(catch_raw_id == "88906") |>
  glimpse()

recapture_tisdale_edi |> 
  mutate(source = "EDI") |>
  filter(catchRawID == "88906") |>
  glimpse()
```
...
recapture_tisdale_db |> 
  mutate(source = "DB") |>
  filter(catch_raw_id == "88904") |>
  glimpse()

recapture_tisdale_edi |> 
  mutate(source = "EDI") |>
  filter(catchRawID == "88904") |>
  glimpse()
```