...
**- Findings catch:**
  ...
  - Date ranges are different, database has an earlier start year (1999 vd 2022 in DB)
  - there are only a few data entries on database with year == 1999 and one data entry with year 2020, wondering if this is expected
...
**- Findings:**
   - time ranges are the same, but time does vary (on "max" value)
...
* `r round(sum(is.na(trap_yuba_edi$waterVel))/nrow(trap_yuba_edi), 3)*100` % of values in the `water_velocity` column are NA.
